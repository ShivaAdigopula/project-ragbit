...
graph TD
    ...
    subgraph External_Services [Local / Private Infrastructure]
        ...
        LLM_Orch <-->|Structured Generation| Ollama[(Ollama Gemma 12B)]
    end
...
* **FR-IDX-02.1 Embedding Generation:** Chunks must be processed using a local embedding model hosted on Ollama (e.g., `nomic-embed-text` producing 768-dimensional vectors).
